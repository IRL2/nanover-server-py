...
from nanover.websocket import NanoverImdClient as NanoverImdClientWS
...
def make_connected_client_from_runner(runner):
    with NanoverImdClientWS.from_runner(runner) as client:
        yield client


@contextmanager
def make_connected_client_from_app_server(app_server: NanoverImdApplication):
    with NanoverImdClientWS.from_app_server(app_server) as client:
        yield client
