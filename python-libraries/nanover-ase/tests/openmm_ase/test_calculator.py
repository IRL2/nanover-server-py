...
def test_energy(basic_simulation):
    ...
    expected_energy = (
        basic_simulation.context.getState(getEnergy=True)
        .getPotentialEnergy()
...
def test_forces(basic_simulation):
    ...
    atoms = calculator.generate_atoms()
    expected_forces = (
        basic_simulation.context.getState(getForces=True)
        .getForces()
        .value_in_unit(kilojoules_per_mole / nanometer)
    )
...
def test_from_serialized_sim(basic_simulation, serialized_simulation_path):
    ...
    assert (
        basic_simulation.system.getNumParticles()
        == calculator.simulation.system.getNumParticles()
    )
...
def test_pbc(basic_simulation):
    ...
    assert np.allclose(
        atoms.get_cell(),
        [
            x.value_in_unit(angstroms)
            for x in calculator.simulation.system.getDefaultPeriodicBoxVectors()
        ],
    )
