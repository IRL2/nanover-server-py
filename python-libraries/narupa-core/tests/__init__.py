...
ARRAYS_STRATEGIES = {
    ...
    "index_values": st.lists(
        st.integers(min_value=0, max_value=MAX_UINT32), min_size=1
    ),
    "float_values": st.lists(
        st.floats(min_value=float(MIN_FLOAT32), max_value=float(MAX_FLOAT32)),
        min_size=1,
    ),
}

MIXED_LIST_STRATEGY = st.lists(
    st.one_of(st.text(), st.booleans(), st.none()), min_size=1
)
MIXED_DICT_STRATEGY = st.dictionaries(
    st.text(), st.one_of(st.text(), st.booleans(), st.none()), min_size=1
)
...
def serializable():
    return st.recursive(
        ...
        lambda s: st.one_of(
            st.lists(s, max_size=5), st.dictionaries(keys(), s, max_size=5)
        ),
        max_leaves=10,
    )
