...
It is the primary software of NanoVer, interfacing with 
...
(Original version)
NanoVer Server is a Python package that facilitates multi-user interactive 
molecular dynamics (iMD) simulations in real-time. It is part of the NanoVer software ecosystem, comprising the
...
simulations and serving them to local and remote clients over the network. This forms the base of NanoVer's 
primary application: the exploration of molecular systems using interactive molecular dynamics in virtual 
reality (iMD-VR), which can be achieved by connecting a dedicated VR client from the NanoVer iMD-VR package.
...
nano-engineering of materials [@lau_nano_engineering_2018].
- Should has been be changed to make it more contemporary
One of the major challenges when using MD simulations to 
explore the dynamics of chemical and biological systems is the sampling of rare events.
- Reword sentence to remove repetition of chemical and biological 
- change sampling?
Many significant molecular 
...
particularly true for biological systems [@hollingsworth_molecular_2018], which often comprise hundreds of thousands of 
...
Interactive molecular dynamics (iMD) is a nonequilibrium approach that allows researchers to bias 
atomic/molecular simulations on-the-fly, without the need to pre-define the behaviour of the system 
[@stone_system_2001; @rapaport_interactive_1997; @oconnor_sampling_2018; @oconnor_interactive_2019]. 
- Nonequilibrium (do we need to explain further)
iMD enables researchers to interact with 
molecular systems in real-time, to bias their dynamics and sample rare events of interest [@stone_system_2001].
- Can we merge the first two sentences?
A number of programs have implementations of iMD, including for example 
...
interact with inherently 3-D systems via haptic interfaces visualized on 2-D screens. Fewer environments are 
available that enable researchers to interact with molecular dynamics simulations in their native 3-D environment, 
which is especially important for exploring complex 
...
fewer support the combination of VR-enabled 3-D visualisation in VR with real-time iMD. The iMD-VR 
combination enables researchers to interact directly with molecular simulations in an environment which is natively 
3-D, where researchers can reach out and ‘touch’ molecular simulations as if they were tangible objects. 
- Native?
...
on iMD-VR. In order to facilitate this, NanoVer operates using a server-client architecture. 
NanoVer Server facilitates iMD by providing an interface between the simulation 
engine used for the molecular system and the researcher(s) connecting to the simulation, equipping
the researcher(s) with tools to interact with the system in real time. NanoVer Server already interfaces with 
multiple programs used to perform molecular simulations [@eastman_openmm_2024; @larsen_atomic_2017], with particular
support for OpenMM. NanoVer Server is written in Python, making it easy to integrate with many of the 
existing tools of the computational chemistry community and beyond. Additionally, being free and open-source, NanoVer 
Server can be customised and extended to interface with other molecular simulation engines.

NanoVer Server performs quantitative iMD simulations, delivering rigorous on-the-fly metrics about both the 
molecular simulation e.g. energies, particle forces and particle velocities, and the perturbations induced by 
the user e.g. the collective user forces, associated potential energy and cumulative work done.
- return to this
A user can tune the relationship between simulation time and real time during
...
slowing the simulation from the user perspective.
...
To achieve quantitative iMD, the server adopts the following blueprint between the publishing of each frame:
  ...
  2) Calculate all iMD forces and energies applied to the system using its current configuration, 
    passing this information to the simulation engine&mdash;these forces will be applied for the next _n_ simulation 
    steps

  3) Compile the data about the current state of the system (including the iMD forces and energies calculated in
    step 2) into a frame and publish to the connected client(s)
...
The server-client architecture of NanoVer enables multiple users to connect simultaneously to a server.
