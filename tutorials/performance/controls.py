...
def show_controls(imd_runner: OmniRunner):
    copied_box: Any = None

    def get_shared_value(key: str, default: Any = None):
        return imd_runner.app_server.copy_state().get(key, default)

    def set_shared_value(key: str, value: Any):
        updates = {key: value}
        change = DictionaryChange(updates=updates)
        imd_runner.app_server.update_state(change)
    ...
    control = [
        ...
        make_state_toggle("suggested.interaction.hydrogens", description="Interactable Hydrogens", default=True),
    ]
    ...
    box = [
        ...
        make_state_toggle("suggested.box.locked", description="Lock Box", default=False),
        make_state_toggle("suggested.box.hidden", description="Hide Box", default=False),
    ]
